...
class RouteRewriteTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $this->expectException(\Solital\Core\Exceptions\NotFoundHttpException::class);

        TestRouter::error(function (\Solital\Core\Http\Request $request, \Exception $error) {
    ...
    public function testRewriteRouteFromRoute()
    {
        ...
        TestRouter::get('/match', function () {
            TestRouter::request()->setRewriteRoute(new \Solital\Core\Course\Route\RouteUrl('/match', function () {
                return 'ok';
            }));
        });
